...
import { defineProps, getCurrentInstance, ref, toRefs, computed, watch, reactive } from "vue";
...
const props = defineProps({
  playId: {
    type: [Number, String], // 歌曲ID 或 歌单ID
    default: 0,
  },
  type: {
...
async function getComment(id) {
  ...
  try {
    const result = (await HttpManager.getAllComment(type.value, normalizedId)) as ResponseBody;
    ...
    for (let index = 0; index < commentList.value.length; index++) {
      ...
      const currentUser = resultUser.data?.[0];
      commentList.value[index].avator = currentUser?.avator || "";
      commentList.value[index].username = currentUser?.username || "匿名用户";
    }
  } catch (error) {
    commentList.value = [];
  }
