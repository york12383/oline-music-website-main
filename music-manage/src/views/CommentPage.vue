...
import { useRoute } from "vue-router"; // ✅ 新增这一行
...
export default defineComponent({
  ...
  setup() {
    ...
    const route = useRoute(); // ✅ 使用 useRoute 替代 proxy.$route
  ...
  promise = HttpManager.getCommentOfSongId(route.query.id as string);
} else if (route.query.type == "1") {
  promise = HttpManager.getCommentOfSongListId(route.query.id as string);
}
